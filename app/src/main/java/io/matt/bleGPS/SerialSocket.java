...
class SerialSocket extends BluetoothGattCallback {
    ...
    private static final UUID GPS_SERVICE_UUID   = UUID.fromString("f7cfe716-cf46-440c-b763-2b10f181051e");
    private static final UUID LOC_CHARACTERISTIC_UUID = UUID.fromString("0a75ba9e-7cf2-490b-9811-f16af03730db");
    private static final UUID SPD_CHARACTERISTIC_UUID = UUID.fromString("f37a98dd-7da6-4578-adae-9dbd481b7034");
    private static final UUID ALT_CHARACTERISTIC_UUID = UUID.fromString("0538d525-011e-450b-a9e4-920010a0cf63");
    ...
    private void connectCharacteristics1(BluetoothGatt gatt) {
        ...
        for (BluetoothGattService gattService : gatt.getServices()) {
            if (gattService.getUuid().equals(GPS_SERVICE_UUID)) {
                Log.d(TAG, "GPS Service Discovered");
                gpsDelegate= new GPSDelegate();
                gpsDelegate.connectCharacteristics(gattService);
            }
        }
    ...
    public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
        gpsDelegate.onDescriptorWrite(gatt,descriptor,status);
    ...
    public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
        if (characteristic.getUuid().equals(LOC_CHARACTERISTIC_UUID) ||
                characteristic.getUuid().equals(SPD_CHARACTERISTIC_UUID) ||
                characteristic.getUuid().equals(ALT_CHARACTERISTIC_UUID)
        ) {
            gpsDelegate.onCharacteristicChanged(gatt,characteristic);
        }
        if(canceled)
            return;
    ...
    private GPSDelegate gpsDelegate;
    private BluetoothGattCharacteristic locCharacteristic;
    private BluetoothGattCharacteristic speedCharacteristic;
    private BluetoothGattCharacteristic altCharacteristic;
    private BluetoothGattDescriptor locDescriptor;
    private BluetoothGattDescriptor speedDescriptor;
    private BluetoothGattDescriptor altDescriptor;
    private class GPSDelegate extends DeviceDelegate {
        boolean connectCharacteristics(BluetoothGattService gattService) {
            Log.d(TAG, "service GPS");

            locCharacteristic = gattService.getCharacteristic(LOC_CHARACTERISTIC_UUID);
            speedCharacteristic = gattService.getCharacteristic(SPD_CHARACTERISTIC_UUID);
            altCharacteristic = gattService.getCharacteristic(ALT_CHARACTERISTIC_UUID);

            if (!gatt.setCharacteristicNotification(locCharacteristic, true)) {
                onSerialConnectError(new IOException("no notification for location characteristic"));
                return false;
            }
            if (!gatt.setCharacteristicNotification(speedCharacteristic, true)) {
                onSerialConnectError(new IOException("no notification for speed characteristic"));
                return false;
            }
            if (!gatt.setCharacteristicNotification(altCharacteristic, true)) {
                onSerialConnectError(new IOException("no notification for altitude characteristic"));
                return false;
            }

            locDescriptor = locCharacteristic.getDescriptor(BLUETOOTH_LE_CCCD);
            if(locDescriptor == null) {
                onSerialConnectError(new IOException("no CCCD descriptor for location characteristic"));
                return false;
            }

            speedDescriptor = speedCharacteristic.getDescriptor(BLUETOOTH_LE_CCCD);
            if(locDescriptor == null) {
                onSerialConnectError(new IOException("no CCCD descriptor for speed characteristic"));
                return false;
            }

            altDescriptor = altCharacteristic.getDescriptor(BLUETOOTH_LE_CCCD);
            if(altDescriptor == null) {
                onSerialConnectError(new IOException("no CCCD descriptor for altitude characteristic"));
                return false;
            }

            int locProperties = locCharacteristic.getProperties();
            if((locProperties & BluetoothGattCharacteristic.PROPERTY_INDICATE) != 0) {
                Log.d(TAG, "enable loc indication");
                locDescriptor.setValue(BluetoothGattDescriptor.ENABLE_INDICATION_VALUE);
            }else if((locProperties & BluetoothGattCharacteristic.PROPERTY_NOTIFY) != 0) {
                Log.d(TAG, "enable loc notification");
                locDescriptor.setValue(BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE);
            }

            int speedProperties = speedCharacteristic.getProperties();
            if((speedProperties & BluetoothGattCharacteristic.PROPERTY_INDICATE) != 0) {
                Log.d(TAG, "enable speed indication");
                speedDescriptor.setValue(BluetoothGattDescriptor.ENABLE_INDICATION_VALUE);
            }else if((speedProperties & BluetoothGattCharacteristic.PROPERTY_NOTIFY) != 0) {
                Log.d(TAG, "enable speed notification");
                speedDescriptor.setValue(BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE);
            }

            int altProperties = altCharacteristic.getProperties();
            if((altProperties & BluetoothGattCharacteristic.PROPERTY_INDICATE) != 0) {
                Log.d(TAG, "enable alt indication");
                altDescriptor.setValue(BluetoothGattDescriptor.ENABLE_INDICATION_VALUE);
            }else if((altProperties & BluetoothGattCharacteristic.PROPERTY_NOTIFY) != 0) {
                Log.d(TAG, "enable alt notification");
        ...
        void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            if (characteristic.getUuid().equals(LOC_CHARACTERISTIC_UUID)) {
                String loc = new String(characteristic.getValue());
                if (listener != null) listener.onUpdateGPSLoc(loc);
                Log.d(TAG, "LOC changed:"+loc);
            }
            else if (characteristic.getUuid().equals(SPD_CHARACTERISTIC_UUID)) {
                String speed = new String(characteristic.getValue());
                if (listener != null) listener.onUpdateGPSSpeed(speed);
                Log.d(TAG, "SPD changed:"+speed);
            }
            else if (characteristic.getUuid().equals(ALT_CHARACTERISTIC_UUID)) {
                String alt = new String(characteristic.getValue());
                if (listener != null) listener.onUpdateGPSAlt(alt);
                Log.d(TAG, "ALT changed:"+alt);
            }
        }
        ...
        void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
            if (descriptor.getCharacteristic() == readCharacteristic) {
                gatt.writeDescriptor(locDescriptor);
            }
            else if (descriptor.getCharacteristic() == locCharacteristic) {
                gatt.writeDescriptor(speedDescriptor);
            } else if (descriptor.getCharacteristic() == speedCharacteristic) {
